...
dbstop if error
save_path = "C:\Users\mazna\Documents\nl\U\P\Code\MatLab\SavedData\PostSeperation\";
...
for dataset = 1:10
    ...
    [final_labelled_map] = PreClassifySeperate2(ct3D,binary_map_filled, CT_dimension_spacing,X_ct, false);
    
    
    save(save_path + num2str(dataset) + "_no_coating.mat", 'final_labelled_map');
    clear final_labelled_map;
    clear ct3D;
...
% [voided, voided_eroded, voided_eroded2] = HardSeperateVolumes(ct3D,binary_map_filled, CT_dimension_spacing, xy_smoothed_thresh,z_thresh);
% 
% 
% comb = voided;
...
% labelled_seperated_binary = labelled_seperated;
% labelled_seperated_binary(labelled_seperated~=0) = 1;
% r1 = 6;
...
% % 
% % 
% % edge_map = binary_map_filled - voided;
% % figure;
% % imshow3D(edge_map);
% % 
% % 
...
% % 
% % labelled_indices = find(labelled_seed_map);
% % indices_labels = labelled_seed_map(labelled_indices);
% % 
% % 
% % min_hard_bone = 1500;
% % 
% % 
% % %in-fill each vol.
% % max_label = max(labelled_seed_map(:));
% % 
% % zero_map = uint8(zeros(size(ct3D)));
% % 
% % label_index_list = cell(max_label,1);
% % 
% % parfor i_label = 1:max_label
% %      tic
% %         label_index_list{i_label,1} = labelled_indices(indices_labels==i_label);
% %      toc
% % end
% % 
% % 
...
% % %     tic
% % %     cur_map = zero_map;
% % %     cur_map(label_index_list{i_label,1}) = 1;
% % %     cur_map = imfill(cur_map,'holes');
% % %     label_index_list{i_label,1} = find(cur_map);
% % %     toc
% % % end
% % 
