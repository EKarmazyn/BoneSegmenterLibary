function score = ScoreEvaluate3(test_ct3D, dog_filt_1,dog_filt_2,pos_points,neg_points, dog_filt_3, dog_filt_4)
    ...
    for i_pp = 1:size(pos_points,1)
        ...
        good_points_scores = [good_points_scores ; dog_filt(cur_point_ind) + dog_filt2(cur_point_ind)];
    end
    ...
    for i_pp = 1:size(neg_points,1)
        ...
        bad_points_scores = [bad_points_scores ; dog_filt(cur_point_ind) + dog_filt2(cur_point_ind)];
    end
    
    
    total_score = mean(good_points_scores) - mean(bad_points_scores);
    factor = 1;
    total_score = total_score + 1*(min(good_points_scores)-max(bad_points_scores));
